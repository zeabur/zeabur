...
你也可以加入我們的 [Discord 服務器](https://discord.gg/DrdGCvXEyY) 以了解更多以及獲得優先支持！

### Zeabur 的優勢

#### 方便快捷

對比傳統的部署方式，Zeabur 可以讓你在幾分鐘內完成部署，而不需要花費大量的時間去學習部署的相關知識，把時間精力專註於產品的開發。
...
Zeabur 采用按量計費的方式，你只需要為你的服務實際用到的資源付費，而不需要為整個服務器付費。

你可以到[定價文檔](/zh-TW/billing/pricing)中了解更詳細的內容。

#### 一站式整合

Zeabur 提供了強大的一站式整合功能。

你可以在一個項目中同時部署前端、後端、數據庫、消息隊列、對象存儲等各種服務，而不需要分別部署到不同的平臺。
