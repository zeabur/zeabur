...
A **Template** can be divided into two main sections: "Template Information" and "Service Specifications". The complete format can be viewed in the [Zeabur Schema Repository](https://json-schema.app/view/%23?url=https%3A%2F%2Fschema.zeabur.app%2Ftemplate.json). Below, each field's purpose and how it is presented on the Zeabur template page will be briefly described.
...
`configs` are the file-based settings of the service. You can use `path` and `template` to specify the default configuration file, making it easier for users to further modify it. For example, with MySQL, you can write the default `configs` as follows:
