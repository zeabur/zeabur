...
除了使用 [Zeabur Dashboard](/template/create-template) 创建模板以外，你也可以用与 [Docker Compose](https://docs.docker.com/compose/) 或 [Kubernetes Object](https://kubernetes.io/docs/concepts/overview/working-with-objects/) 相似的形式，使用 `zeabur` CLI 从 YAML 部署、创建和管理模板。
...
`configs` 是服务的文件型配置，可以使用 `path` 和 `template` 指定默认的配置文件，方便用户进一步修改。也可以使用 `permission` 来指定文件的权限。以 MySQL 为例，你可以这样编写默认的 `configs`：
...
注意 `permission` 必须是一个十进制数字，由八进制的 [UNIX 文件权限](https://mason.gmu.edu/~montecin/UNIXpermiss.htm) 转换而来。以下是几个常见的权限对照表：

| `permission` 值 | 权限原始值 | 读  | 写  | 执行 | 适合场景                       |
| --------------- | ---------- | --- | --- | ---- | ------------------------------ |
| 256             | 0400       | O   | X   | X    | 机密文件（如密码）             |
| 420             | 0644       | O   | O   | X    | 普通可读写文件，默认权限        |
| 493             | 0755       | O   | O   | O    | 可执行文件（如 bash 脚本）     |

注意这里的“读、写、执行”均指容器用户的权限。详细的信息（组、everyone 等）可以参考上方网址。

`domainKey` 标示模板定义中的域名 (`DOMAIN`) 变量应该绑定在哪个服务上面。就以上面的例子来说，`spec.variables` 有一个类型是 `DOMAIN` 的变量 `PUBLIC_DOMAIN`，而 RSSHub 服务规格下有一个 `domainKey` 指向 `PUBLIC_DOMAIN`，届时部署时将会把 `PUBLIC_DOMAIN` 设定的域名绑定到 RSSHub 服务上。
...
假如测试上没有问题，你可以使用 `npx zeabur template create` 命令将模板公开给其他人使用。同样以上文的 `wp.yaml` 为例：
...
如果你需要修改模板的内容，比如改名、更新 Image 版本，你可以在修改完成 Resource YAML 后，使用 `npx zeabur template update` 命令更新：
