...
如果是 `PREBUILT` 服务，则需要指定 Docker image (`image`)，以及选择性的运行命令和运行参数 (`command` 和 `args`)；如果是 `GIT` 服务，则需要指定 Git repository 的类型（目前只支持 `GITHUB`）、存储库的代号（目前只支持 GitHub 的 [`repoID`](https://stackoverflow.com/a/47223479)），亦可选择性填写分支 (`branch`)。
...
`configs` 是服务的文件型设置，可以使用 `path` 和 `template` 指定默认的设置文件，方便用户进一步修改。以 MySQL 为例，你可以这样编写默认的 `configs`：
