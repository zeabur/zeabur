...
| `html` | string | No* | Cuerpo del correo en formato HTML. Admite formato enriquecido como negrita, colores, imágenes y enlaces. Máximo 5 MB. |
| `text` | string | No* | Cuerpo del correo en texto plano, sin formato. Máximo 5 MB. |
...
<Callout type="warning">
  Se debe proporcionar al menos uno de `html` o `text`. Si se proporcionan ambos, HTML tiene prioridad. El total de destinatarios (to + cc + bcc) no puede exceder 50.
</Callout>
...
| `job_type` | string | Filtrar por origen: `direct` (envío único), `scheduled` (envío programado), `batch` (envío por lotes) |
| `job_id` | string | Filtrar por ID de trabajo (ID de correo programado o ID de trabajo por lotes) |
...
| Campo | Tipo | Descripción |
|-------|------|-------------|
| `job_type` | string | Origen del correo: `direct` (envío único), `scheduled` (envío programado), `batch` (envío por lotes) |
| `job_id` | string | ID de trabajo. Vacío para envíos directos; ID del correo programado o ID del trabajo por lotes |
