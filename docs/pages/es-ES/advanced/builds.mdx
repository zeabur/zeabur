...
In the design of zbpack, Docker Image is the most basic output format, which ensures that we can always build a service that can run. But in addition to this, zbpack also supports the output formats of static resources and cloud functions. For example, when we use [Vite](https://vitejs.dev/) to develop a static website:
...
Therefore, the command to start this service has changed from the original `docker run -p 8080:8080 -it nextjs-template` to `npx serve .zeabur/output/static`. Similarly, we can put these static resources into any static web server such as [Nginx](https://www.nginx.com/), [Apache](https://httpd.apache.org/), [Caddy](https://caddyserver.com/) and so on.
