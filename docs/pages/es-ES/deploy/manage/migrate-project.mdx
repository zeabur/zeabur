...
Migrar un proyecto es esencialmente **copiar el proyecto + sincronizar datos**. La función de [Copiar Proyecto](/deploy/manage/copy-project) de Zeabur le permite duplicar un proyecto completo (incluyendo configuraciones de servicio, variables de entorno y datos de volúmenes) a otra región con un solo clic. Combinado con [Copia de seguridad y Restauración](/operations/data/backup-restore), puede realizar una migración completa entre regiones o incluso entre cuentas.
...
Para pasos detallados, consulte la documentación de [Copiar Proyecto](/deploy/manage/copy-project).
...
1. **Exportar la configuración del proyecto** — Utilice la función de [Exportar Proyecto](/deploy/manage/export-project) para descargar la configuración YAML del proyecto.
2. **Hacer copia de seguridad de los datos** — Cree copias de seguridad para todos los servicios con estado:
   - Bases de datos: Utilice la función de copia de seguridad en línea en [Copia de seguridad y Restauración](/operations/data/backup-restore).
   - Almacenamiento persistente: Utilice copia de seguridad fuera de línea o descargue manualmente a través de [Gestión de Archivos](/operations/data/file-management).
...
- **Integración Git** — Si los servicios están vinculados a un repositorio de GitHub, deberá reconfigurar la [integración con GitHub](/deploy/methods/github-integration) en la nueva cuenta después de la migración entre cuentas.
