...
Si construyes tu imagen Docker localmente en un Mac con Apple Silicon (M1/M2/M3/M4), la imagen se genera por defecto para la plataforma `linux/arm64`.
Los servidores de Zeabur ejecutan `linux/amd64`, por lo que desplegar una imagen solo ARM fallará con el error **"no match for platform in manifest"**.

Para evitar este problema, construye tu imagen con el flag `--platform linux/amd64`:
...
Además de las variables expuestas por otros servicios, también puedes usar [variables especiales predefinidas de Zeabur](/deploy/config/environment-variables), como usar `${ZEABUR_WEB_URL}` para obtener la URL establecida en el puerto `web`.
...
Al especificar desde la GUI, el punto de entrada predeterminado al configurar este campo es `/bin/sh`. Si tu imagen no proporciona `/bin/sh`, usa **Editar TOML** para cambiarlo o consulta [soporte](/get-started/faq-support/help) para ayuda.

## Escribe manualmente tu plantilla

Aunque la GUI ofrece muchos parámetros necesarios para crear tus servicios Docker, es posible que desees añadir iconos personalizados, instrucciones de conexión o configuraciones detalladas. En este caso, [escribir tu plantilla en código](/template/template-format) es una mejor opción.

Si tienes alguna pregunta sobre cómo escribir tu propia plantilla, no dudes en contactar con [soporte](/get-started/faq-support/help).
