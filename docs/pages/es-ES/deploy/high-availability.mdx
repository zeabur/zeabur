...
<Callout type="info">
  Esta arquitectura de alta disponibilidad se aplica tanto a **Servidores Dedicados** como a **Clústeres Dedicados**. Sin embargo, si desea conmutación automática por error a un nodo saludable cuando un nodo falla (es decir, configurar múltiples nodos Kubernetes), necesitará un **Clúster Dedicado**. Zeabur también está [eliminando gradualmente los clústeres compartidos](https://zeabur.com/changelogs/phasing-out-shared-cluster) — los nuevos proyectos deben usar Servidores Dedicados o Clústeres Dedicados.
</Callout>
...
Tenga en cuenta que este es un tema avanzado. Típicamente, no necesita alta disponibilidad (HA) para los servicios de su startup. Para la solución más sencilla, coloque sus servicios en un solo proyecto y use [redes privadas](/deploy/networking/private-networking) para la comunicación. Exponga su servicio a Internet utilizando [redes públicas](/deploy/networking/public-networking). Nosotros podemos encargarnos del escalado vertical por usted.
