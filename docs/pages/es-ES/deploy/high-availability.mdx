...
<Callout type="info">
  Esta arquitectura de alta disponibilidad se aplica tanto a **Servidores** como a **Clústeres Dedicados**. Sin embargo, si desea conmutación automática por error a un nodo saludable cuando un nodo falla (es decir, configurar múltiples nodos Kubernetes), necesitará un **Clúster Dedicado**. Zeabur también está [eliminando gradualmente los clústeres compartidos](https://zeabur.com/changelogs/phasing-out-shared-cluster) — los nuevos proyectos deben usar Servidores o Clústeres Dedicados.
</Callout>
