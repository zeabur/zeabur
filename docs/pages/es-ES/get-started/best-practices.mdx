...
Almacena todos los valores de configuración — claves API, URLs de bases de datos, feature flags — en [variables de entorno](/deploy/config/environment-variables). Nunca codifiques secretos directamente en tu código fuente o imágenes Docker.

<Callout type="warning">
Los secretos commiteados en Git quedan permanentemente expuestos en el historial de tu repositorio. Usa siempre la gestión de variables de entorno de Zeabur.
</Callout>
...
Los [volúmenes persistentes](/operations/data/volumes) son esenciales para bases de datos y almacenamiento de archivos, pero tienen un trade-off: los servicios con volúmenes adjuntos no pueden realizar despliegues rolling sin tiempo de inactividad.
...
## Usa Plantillas de Bases de Datos Preconfiguradas

En lugar de ejecutar tus propios contenedores de bases de datos, usa las [plantillas de bases de datos de un clic](/template) de Zeabur. Vienen preconfiguradas, incluyen inyección automática de variables de conexión y están optimizadas para la plataforma.

Las opciones populares incluyen PostgreSQL, MySQL, MongoDB y Redis — todas desplegables con un clic.
...
<Callout type="info">
Para proyectos que no están en un repositorio Git, también puedes desplegar vía [CLI](/developer/cli) o imágenes Docker.
</Callout>

## Habilita Dominios Personalizados con HTTPS

Zeabur proporciona certificados TLS automáticos para todos los dominios personalizados. Apunta tu DNS a Zeabur y HTTPS se habilita con cero configuración — consulta [Dominios Personalizados](/domain/management) para instrucciones de configuración.
...
- Usa la función de [respaldo y restauración](/operations/data/backup-restore) para snapshots de bases de datos.
- Exporta configuraciones importantes vía [gestión de archivos de configuración](/operations/data/config-file-management).
- Almacena respaldos externamente (por ejemplo, almacenamiento de objetos) para recuperación ante desastres.

<Callout type="info">
Combinar estas prácticas con las funciones integradas de Zeabur — como [alta disponibilidad](/deploy/networking/high-availability) — te da una base sólida para cargas de trabajo de producción.
</Callout>
