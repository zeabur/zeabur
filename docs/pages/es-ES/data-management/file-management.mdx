...
Actualmente, la Gestión de Archivos no admite eliminar carpetas completas a través de la interfaz. Puedes ejecutar el comando `rm` para eliminar carpetas completas usando [Ejecución de Comandos](/deploy/config/command-execution). Aquí hay un ejemplo de usar `rm -rf` para eliminar toda la carpeta `/tmp/testdir`:
