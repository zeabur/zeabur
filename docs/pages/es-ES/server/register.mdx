...
ogImageTitle: Registrar Servidor
...
Este artículo explicará cómo registrar un [Servidor](../server) en Zeabur. Si no tienes tu propio servidor, puedes [comprar uno en Zeabur](./purchase) en su lugar.
...
1. Después de iniciar sesión en Zeabur, haz clic en "Mis Servidores" en la barra de navegación superior o en la parte superior de la página de la lista de proyectos, o visita directamente [https://zeabur.com/servers](https://zeabur.com/servers).
...
<Callout type="warning">
    Zeabur solo acepta claves SSH en **formato OpenSSH** (la clave privada comienza con `-----BEGIN OPENSSH PRIVATE KEY-----`). Algunos proveedores de nube, como Tencent Cloud, proporcionan claves SSH en el formato heredado RSA PEM (`-----BEGIN RSA PRIVATE KEY-----`) por defecto, lo que causará un fallo en la conexión SSH. Para convertir tu clave al formato OpenSSH, ejecuta:

    ```bash
    ssh-keygen -p -N "" -f /path/to/your_private_key
    ```

    Después de la conversión, pega el contenido actualizado de la clave en el campo de clave SSH.
</Callout>
...
Si encuentras un fallo de registro, infórmanos en el [foro de la comunidad](https://zeabur.com/forum).
