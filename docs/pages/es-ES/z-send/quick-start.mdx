---
title: Inicio Rápido
ogImageTitle: Inicio Rápido de Z-Send
ogImageSubtitle: Envíe su Primer Correo en 5 Minutos
---

# Inicio Rápido

Esta guía le ayudará a enviar su primer correo usando Z-Send en minutos.

## Requisitos Previos

Antes de comenzar, necesitará:

1. Una cuenta de Zeabur
2. Servicio Z-Send habilitado en su consola de Zeabur
3. Al menos un dominio de remitente verificado
4. Una clave API

## Paso 1: Crear una Clave API

import { Callout, Steps } from 'nextra/components'

<Steps>

### Iniciar Sesión en la Consola de Zeabur

Visite [Zeabur Dashboard](https://dash.zeabur.com) e inicie sesión en su cuenta.

### Navegar a la Gestión de Z-Send

Encuentre el servicio Z-Send en la consola y entre a la página de gestión.

### Crear una Nueva Clave API

1. Haga clic en el botón "Crear Clave API"
2. Seleccione el tipo de permiso:
   - **Solo Lectura**: Solo puede consultar el estado del correo
   - **Solo Envío**: Puede enviar correos y consultar el estado (recomendado para producción)
   - **Acceso Completo**: Incluye todos los permisos de operación
3. Opcional: Restringir permisos de dominio (solo puede enviar desde dominios específicos)
4. Haga clic en "Crear"

<Callout type="warning">
  ¡Guarde su clave API inmediatamente después de crearla - solo se mostrará una vez!
</Callout>

</Steps>

## Paso 2: Verificar su Dominio

Para enviar correos, primero necesita verificar su dominio de remitente:

<Steps>

### Agregar Dominio

En la sección "Gestión de Dominios" de la página de gestión de Z-Send, haga clic en "Agregar Dominio".

### Configurar Registros DNS

El sistema generará registros DKIM, SPF y DMARC. Necesita agregar estos registros en el proveedor DNS de su dominio.

### Esperar la Verificación

Después de agregar los registros DNS, haga clic en el botón "Verificar". La verificación generalmente toma de unos minutos a unas horas.

</Steps>

## Paso 3: Enviar su Primer Correo

¡Una vez que su dominio esté verificado, estará listo para enviar correos!

### Usando cURL

```bash
curl -X POST https://gateway.zeabur.com/api/v1/zsend/emails \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "from": "hello@yourdomain.com",
    "to": ["user@example.com"],
    "subject": "¡Hola desde Z-Send!",
    "html": "<h1>¡Hola!</h1><p>Este es mi primer correo desde Z-Send.</p>",
    "text": "¡Hola! Este es mi primer correo desde Z-Send."
  }'
```

### Usando JavaScript/Node.js

```javascript
const response = await fetch('https://gateway.zeabur.com/api/v1/zsend/emails', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer YOUR_API_KEY'
  },
  body: JSON.stringify({
    from: 'hello@yourdomain.com',
    to: ['user@example.com'],
    subject: '¡Hola desde Z-Send!',
    html: '<h1>¡Hola!</h1><p>Este es mi primer correo desde Z-Send.</p>',
    text: '¡Hola! Este es mi primer correo desde Z-Send.'
  })
});

const data = await response.json();
console.log('Correo enviado:', data);
```

### Usando Python

```python
import requests

response = requests.post(
    'https://gateway.zeabur.com/api/v1/zsend/emails',
    headers={
        'Content-Type': 'application/json',
        'Authorization': 'Bearer YOUR_API_KEY'
    },
    json={
        'from': 'hello@yourdomain.com',
        'to': ['user@example.com'],
        'subject': '¡Hola desde Z-Send!',
        'html': '<h1>¡Hola!</h1><p>Este es mi primer correo desde Z-Send.</p>',
        'text': '¡Hola! Este es mi primer correo desde Z-Send.'
    }
)

print('Correo enviado:', response.json())
```

### Usando Go

```go
package main

import (
    "bytes"
    "encoding/json"
    "fmt"
    "net/http"
)

func main() {
    payload := map[string]interface{}{
        "from":    "hello@yourdomain.com",
        "to":      []string{"user@example.com"},
        "subject": "¡Hola desde Z-Send!",
        "html":    "<h1>¡Hola!</h1><p>Este es mi primer correo desde Z-Send.</p>",
        "text":    "¡Hola! Este es mi primer correo desde Z-Send.",
    }

    jsonData, _ := json.Marshal(payload)
    req, _ := http.NewRequest("POST", "https://gateway.zeabur.com/api/v1/zsend/emails", bytes.NewBuffer(jsonData))
    req.Header.Set("Content-Type", "application/json")
    req.Header.Set("Authorization", "Bearer YOUR_API_KEY")

    client := &http.Client{}
    resp, err := client.Do(req)
    if err != nil {
        panic(err)
    }
    defer resp.Body.Close()

    fmt.Println("¡Correo enviado! Estado:", resp.Status)
}
```

## Ejemplo de Respuesta

Después de enviar con éxito, recibirá una respuesta como esta:

```json
{
  "email_id": "696de2c84210d814d66ee052",
  "status": "queued",
  "message": "Email queued for sending"
}
```

## Próximos Pasos

¡Ha enviado su primer correo con éxito! Ahora puede:

- Consultar la [Referencia de API REST](/es-ES/z-send/rest-api) para conocer más funciones
- Configurar [Webhooks](/es-ES/z-send/webhooks) para recibir notificaciones de estado de correo
- Aprender cómo enviar [correos con archivos adjuntos](/es-ES/z-send/rest-api#enviar-correos-con-archivos-adjuntos)
- Explorar [envío programado](/es-ES/z-send/rest-api#programar-correos) y [envío por lotes](/es-ES/z-send/rest-api#envío-por-lotes)

<Callout type="info">
  ¿Tiene problemas? Consulte nuestra guía de solución de problemas o contacte con el soporte.
</Callout>
