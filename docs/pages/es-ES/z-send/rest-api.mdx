...
ogImageTitle: Referencia de REST API de Z-Send
ogImageSubtitle: Documentación Completa de Endpoints API
---

# Referencia de REST API

Z-Send proporciona una API REST completa que facilita la integración de funcionalidad de envío de correos en su aplicación.
...
<Callout type="info">
  Las claves API se pueden crear y gestionar en la página de gestión de Z-Send de la consola de Zeabur.
</Callout>
...
curl -X POST https://api.zeabur.com/api/v1/zsend/emails \
  ...
  -d '{
    ...
    "html": "<h1>¡Bienvenido!</h1><p>Gracias por usar Z-Send.</p>"
  }'
```
...
Igual que el envío de correo individual, con un campo adicional:

```json
{
  // ... otros campos iguales que /emails
  "scheduled_at": "2026-01-25T10:00:00Z"  // Formato ISO 8601
}
```
...
<Callout type="info">
  Un envío por lotes admite hasta 100 correos. Cada correo tiene los mismos campos que el envío de correo individual.
</Callout>
...
Configure Webhooks para recibir actualizaciones de estado de correo en tiempo real en lugar de sondear la interfaz de consulta. Consulte [Configuración de Webhooks](/es-ES/z-send/webhooks) para más detalles.
