...
目前，[zbpack](https://github.com/zeabur/zbpack) 已經支援所有網頁開發主流的程式語言，並對各個較熱門的開發框架實作了進一步的識別及最佳化，同時也在不斷為新出現的程式語言和框架推出更新：
...
這個時候，由於計畫中找到了 `outputDir`，zbpack 會自動將構建結果輸出到 `.zeabur/output/static` 目錄中：
...
關於 zbpack 的 `.zeabur/output` 目錄的更多細節，請參考 [Serverless 輸出格式](./serverless-output-format) 章節。
