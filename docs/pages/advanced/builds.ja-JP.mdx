...
この時、`outputDir`がプランにあるので、zbpackは自動的にビルド結果を`.zeabur/output/static`フォルダに出力します：
...
したがって、このサービスを開始するコマンドは、元の`docker run -p 8080:8080 -it nextjs-template`から`npx serve .zeabur/output/static`に変更されています。同様に、これらの静的リソースを、[Nginx]()、[Apache]()、[Caddy]()などの任意の静的ウェブサーバーに置くことができる。

zbpackの`.zeabur/output`フォルダの詳細については、[Serverless Output Format](./serverless-output-format)のセクションを参照してください。
