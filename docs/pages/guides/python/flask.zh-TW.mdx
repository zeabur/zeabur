...
Zeabur 使用 `python app.py` 或 `python main.py` 來啟動 Flask 應用程式，因此你的專案應包含這兩個文件之一並包含運作 Flask 應用程式所需的指令。
...
另 Zeabur 使用 [Gunicorn](https://gunicorn.org) 作為執行 Django 服務的 WSGI 生產伺服器。
