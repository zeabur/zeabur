...
他のフレームワークで作成されたプロジェクト（または以下のようにカスタムサーバーレス形式の場合）、現在は手動でオプトインが必要です。
サーバーレスサポートを有効にするには、[サーバーレスを有効にする](/deploy/serverless)ページを参照してください。
テスト中に問題がない場合、お気軽にあなたのフレームワークをサポートするために [zbpack コードリポジトリ](https://github.com/zeabur/zbpack) にプルリクエストを送信してください。
