...
デフォルトでは、Zeaburは依存関係をインストールする手順をキャッシュし、その後のCI/CDプロセスを高速化します。ほとんどのNode.jsプロジェクトには影響しないはずですが、pnpm-workspaceなどのモノレポを管理するツールを使用しており、デフォルトの設定がコンパイルエラーを引き起こす場合、キャッシュ機能を無効にして正しく使用する必要があるかもしれません。
