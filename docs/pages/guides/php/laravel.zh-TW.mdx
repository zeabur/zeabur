...
2. [建立 MySQL 服務](/marketplace/mysql)，然後在你的本機環境執行以下命令將 table 等設定到 Zeabur 上。
...
3. 在你 Zeabur 上的 Laravel 服務頁面，建立一個 DATABASE_URL 環境變數，內容如下：
    ...
    `${MYSQL_USER}` 等變數的意義可以參考 [MySQL 文件的環境變數一章](https://zeabur.com/docs/zh-TW/marketplace/mysql#環境變數)。
