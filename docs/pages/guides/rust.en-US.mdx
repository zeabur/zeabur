...
Unlike other PaaS platforms that typically package your Rust application in Docker containers for execution, Zeabur also supports
[deploying Rust serverlessly](../deploy/serverless). This allows your Rust application to only start when it receives a request, rather than continuously running like a container, wasting resources.
