...
<Callout type="info">
  この高可用性アーキテクチャは、**専用サーバー**と**専用クラスター**の両方に適用されます。ただし、水平スケーリング（サービスレプリカ）には**専用クラスター**（マルチノード Kubernetes クラスター）が必要です。Zeabur は[共有クラスターを段階的に廃止](https://zeabur.com/changelogs/phasing-out-shared-cluster)しています。新規プロジェクトは専用サーバーまたは専用クラスターをご利用ください。
</Callout>
