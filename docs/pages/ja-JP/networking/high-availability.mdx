...
<Callout type="info">
  この高可用性アーキテクチャは、**専用クラスター**（マルチノード Kubernetes クラスター）のユーザー向けです。シングルノードの**専用サーバー**には適用されません。Zeabur は[共有クラスターを段階的に廃止](https://zeabur.com/changelogs/phasing-out-shared-cluster)しています。新規プロジェクトは専用サーバーまたは専用クラスターをご利用ください。
</Callout>
