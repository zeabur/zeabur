...
他のユーザーが NGINX サービスを通じて内部の Vaultwarden サービスにアクセスできるようにするため、NGINX で `proxy_pass http://vaultwarden.zeabur.internal:80;` ディレクティブを使用しています。これは、NGINX がこの場所へのすべてのリクエストを Vaultwarden にプロキシすることを示しています。[プライベートネットワーク](/deploy/networking/private-networking)機能を使用して、固定ホスト名でプライベートネットワーク内の Vaultwarden にアクセスしています。
...
[NGINX サービスにドメインをバインド](/deploy/networking/public-networking)した後、ドメインを通じて Vaultwarden サービスにアクセスできます。

![Access Vaultwarden](/data-management/config-edit/vultwarden-service.en-US.png)

## テンプレート YAML での設定ファイルの事前指定

「サービス仕様」で `configs` を使用して、このサービスの設定ファイルを事前に指定できます。詳細については、[YAML からテンプレートを作成する](/template/template-format)の記事をご参照ください。
