...
| `html` | string | いいえ* | HTML 形式のメール本文。太字・カラー・画像・リンクなどのリッチなフォーマットに対応。最大5 MB。 |
| `text` | string | いいえ* | プレーンテキスト形式のメール本文（書式なし）。最大5 MB。 |
| `attachments` | array | いいえ | 添付ファイルリスト（最大10個、各最大10 MB） |
| `headers` | object | いいえ | カスタムメールヘッダー（最大50個） |
| `tags` | object | いいえ | 分類と追跡用のカスタムタグ |

<Callout type="warning">
  `html` と `text` のどちらか一方は必須です。両方指定した場合、HTML が優先されます。合計受信者数（to + cc + bcc）は50人を超えることはできません。
</Callout>
...
| `job_type` | string | ソースでフィルタ：`direct`（単一送信）、`scheduled`（スケジュール送信）、`batch`（バッチ送信） |
| `job_id` | string | ジョブ ID でフィルタ（スケジュールメール ID またはバッチジョブ ID） |
...
| フィールド | タイプ | 説明 |
|-----------|-------|------|
| `job_type` | string | メールソース：`direct`（単一送信）、`scheduled`（スケジュール送信）、`batch`（バッチ送信） |
| `job_id` | string | ジョブ ID。単一送信の場合は空；スケジュールメール ID またはバッチジョブ ID |
...
リアルタイムのメールステータス更新を受信するには、クエリインターフェースをポーリングするのではなく、Webhook を設定してください。詳細は [Webhook 設定](/ja-JP/email/webhooks) を参照してください。
