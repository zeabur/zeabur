...
<Callout type="info">
  サービスが [Wonder Mesh](/wonder-mesh) サーバーにデプロイされている場合、ドメインやポートフォワーディングなしでローカルネットワークから直接アクセスすることもできます。詳細は [LAN からのサービスアクセス](/wonder-mesh/get-started#accessing-services)を参照してください。
</Callout>
...
[Docker サービスの設定](/deploy/methods/custom-docker-image) 時にポートを設定できます。詳細については、Docker サービス設定ドキュメントの「Ports」セクションを参照してください。
