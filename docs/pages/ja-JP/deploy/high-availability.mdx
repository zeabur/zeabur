...
<Callout type="info">
  この高可用性アーキテクチャは、**専用サーバー**と**専用クラスター**の両方に適用されます。ただし、単一ノード障害時に正常なノードへ自動フェイルオーバーする（複数の Kubernetes ノードを構成する）には、**専用クラスター**が必要です。Zeabur は[共有クラスターを段階的に廃止](https://zeabur.com/changelogs/phasing-out-shared-cluster)しています。新規プロジェクトは専用サーバーまたは専用クラスターをご利用ください。
</Callout>

このドキュメントでは、Zeaburでサービスを高可用性でデプロイするための推奨アーキテクチャの概要を説明します。

このトピックはやや高度であることに注意してください。新しいサービスの場合、通常、高可用性（HA）は必要ありません。最も簡単な解決策は、サービスを1つのプロジェクトに配置し、[プライベートネットワーク](/deploy/private-networking)を使用して通信することです。[パブリックネットワーク](/deploy/public-networking)を使用してインターネットにサービスを公開します。Zeaburは垂直スケーリングを処理できます。
