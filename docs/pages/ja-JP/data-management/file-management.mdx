...
The edited file content will also be placed in the [Config Editor](/data-management/config-edit), ensuring that changes are retained after service restarts. To revert a file to its original state, remove the corresponding item in the Config Editor.
