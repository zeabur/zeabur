...
現在、Zeaburはインターフェースでフォルダ全体を削除することをサポートしていません。[コマンド実行](/deploy/config/command-execution)機能と`rm`コマンドを組み合わせてフォルダ全体を削除できます。以下は`rm -rf`を使用して`/tmp/testdir`フォルダ全体を削除する例です：
...
次に[コマンド実行](/deploy/config/command-execution)機能を使用し、以下のコマンドを入力して`tar`ファイルの内容を確認します：
