...
[Zeabur Dash](https://zeabur.com/new)でプロジェクトを作成します。

## ステップ2：サービスの作成

上記のプロジェクトで、NGINXとVaultwardenのPrebuiltサービスをそれぞれ作成します。

![NGINXサービスの作成](/data-management/config-edit/nginx.png)

![Vaultwardenサービスの作成](/data-management/config-edit/vaultwarden.png)
...
![設定ファイルエディタを開く](/data-management/config-edit/nginx_open_config.png)
...
![設定ファイルの追加](/data-management/config-edit/nginx_create_config_file.png)
...
![設定ファイルの保存](/data-management/config-edit/Open_Config_Editor.png)

## ステップ4：サービスの再起動

上記の手順で設定ファイルの編集が完了しましたので、次はサービスを再起動する必要があります。設定ファイルを有効にするために、サービス状態の `Restart` ボタンをクリックしてサービスを再起動してください。

![サービスの再起動](/data-management/config-edit/restart_nginx_Instance.png)
