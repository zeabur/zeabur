...
ogImageTitle: Z-Send REST API リファレンス
ogImageSubtitle: 完全な API エンドポイントドキュメント
---

# REST API リファレンス

Z-Send は、アプリケーションにメール送信機能を簡単に統合できる完全な REST API を提供します。
...
<Callout type="info">
  API キーは、Zeabur コンソールの Z-Send 管理ページで作成および管理できます。
</Callout>
...
curl -X POST https://api.zeabur.com/api/v1/zsend/emails \
  ...
  -d '{
    ...
    "html": "<h1>ようこそ！</h1><p>Z-Send をご利用いただきありがとうございます。</p>"
  }'
```
...
単一メール送信と同じで、追加フィールドがあります：

```json
{
  // ... /emails と同じ他のフィールド
  "scheduled_at": "2026-01-25T10:00:00Z"  // ISO 8601 形式
}
```
...
<Callout type="info">
  単一のバッチ送信は最大100通のメールをサポートします。各メールには単一メール送信と同じフィールドがあります。
</Callout>
