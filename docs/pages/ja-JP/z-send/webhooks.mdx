...
メールのステータスが変更されると、Z-Send は設定された URL にイベントの詳細を含む HTTP POST リクエストを送信します。
...
リクエストが Z-Send から送信されたことを確認するには、`X-ZSend-Signature` を検証する必要があります。
...
Z-Send は指数バックオフを使用して最大 **3回** 自動的にリトライします：
...
<Callout type="info">
  Z-Send コンソールで Webhook ログを表示して、送信ステータスとレスポンスの詳細を理解することをお勧めします。
</Callout>
