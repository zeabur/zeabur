...
zbpackの設計では、Dockerイメージは最も基本的な出力形式であり、いつでも実行できるサービスをビルドすることができます。しかし、それに加えて、zbpackは静的リソースやクラウド関数の出力形式もサポートしています。たとえば、[Vite](https://vitejs.dev/)を使用して静的ウェブサイトを開発するとき：
...
したがって、このサービスを開始するコマンドは、元の`docker run -p 8080:8080 -it nextjs-template`から`npx serve .zeabur/output/static`に変更されています。同様に、これらの静的リソースを、[Nginx](https://www.nginx.com/)、[Apache](https://httpd.apache.org/)、[Caddy](https://caddyserver.com/)などの任意の静的ウェブサーバーに置くことができる。
