...
必要な API がこの Schema に含まれていない場合は、[community forum](https://zeabur.com/forum) で私たちにお知らせください。

## チームが所有するリソース

API キーは、あなたの**個人アカウント**に紐づいています。チームは独自の API キーを持ちません。同じキーで、あなたが所属するいずれのチームのリソースにもアクセスできますが、そのチームでのロールが持つ権限の範囲に制限されます。

リソース（プロジェクトなど）は、あなたの個人アカウントまたはチームが所有できます。所有されたリソースを操作するクエリおよびミューテーションは、`ownerID` 引数を受け付けます。チームの ID を渡すとそのチームに対して操作を行い、省略すると個人アカウントに対して操作を行います。たとえば、チームのプロジェクト一覧を取得するには次のようにします：
...
あなたが所属するチームとその ID を確認するには、`teams` クエリを使用します：
...
<Callout type="info">
Zeabur CLI をお使いですか？CLI ではこれをワークスペースという概念でラップしています。詳しくは CLI ドキュメントの [ワークスペース（チーム）](./cli#ワークスペースチーム) を参照してください。
</Callout>
...
<Callout>
WebSocket 経由のリアルタイム GraphQL サブスクリプション（ログ、プロジェクトアクティビティなど）については、詳細な [WebSocket 接続ガイド](./websocket-guide) を参照してください。
</Callout>
