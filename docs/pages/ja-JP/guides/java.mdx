...
Java プロジェクト全体（`pom.xml` や `build.gradle` を含む）が Git リポジトリのサブディレクトリにある場合は、代わりに[ルートディレクトリ](/deploy/config/root-directory)設定を使用してください。これにより、Zeabur はそのサブディレクトリをプロジェクトルートとして扱い、デフォルトの JAR 検出が正常に機能します。
