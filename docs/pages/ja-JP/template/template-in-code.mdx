...
## `zeabur` CLI を使用したテンプレートの削除

重複するテンプレートがある場合や、もうテンプレートが必要ない場合は、`npx zeabur@latest template delete` コマンドを使用して削除できます：
...
コマンドを実行し、プロンプトが表示されたらテンプレートコードを入力してください。テンプレートはプラットフォームから完全に削除されます。
