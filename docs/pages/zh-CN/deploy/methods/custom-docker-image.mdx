...
如果你在 Apple Silicon Mac（M1/M2/M3/M4）上本地构建 Docker 镜像，镜像默认会是 `linux/arm64` 平台。
Zeabur 的服务器运行在 `linux/amd64` 上，因此部署仅包含 ARM 架构的镜像会失败，并显示 **"no match for platform in manifest"** 错误。

为避免此问题，请在构建时加上 `--platform linux/amd64` 参数：
...
除了其他服务 Exposed 的变量以外，你也可以使用 [Zeabur 预先定义好的特殊变量](/deploy/config/environment-variables)，比如使用 `${ZEABUR_WEB_URL}` 取回设置在 `web` port 的网址。
...
“Port Type”可以是 `HTTP` 或 `TCP`。外网的部分，`HTTP` 可以使用域名（如 `my-service.zeabur.app`）直连，同时 Zeabur 也会自动签发 TLS 证书；`TCP` 则可以使用 Zeabur 自动分配的转发主机名和 Port 连接（如 `reg.clusters.zeabur.com:12345`）。如果是服务之间，可以使用 [服务内网](/deploy/networking/private-networking) 直连到服务，减少不必要的流量费用，此时 Port 直接填写你声明的 Port 即可（亦即 `mariadb.zeabur.internal:3306`、`backend.zeabur.internal:8080`）。
...
从 GUI 创建的 Start Command 默认的 Command 都是 `/bin/sh`。如果你的 Command 不是 `/bin/sh`，可使用 Edit TOML 功能直接修改，或与 [客服](/get-started/faq-support/help) 讨论更改方式。

## 编写自己的模板 YAML

虽然 GUI 提供了许多用于创建您的 Docker 服务的必要参数，但您可能希望添加自定义图标、连接说明或详细设置。在这种情况下，[以代码编写您的模板](/template/template-format) 可能是更好的选择。

如果您对于编写自己的模板有任何问题，欢迎联系[客服](/get-started/faq-support/help)。
