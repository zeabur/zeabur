...
<Callout type="info">
  此高可用性架构适用于**专用服务器**和**专用集群**。但如果想要在单个节点故障时自动转移到另一个正常的节点（也就是配置多个 Kubernetes 节点），会需要**专用集群**。Zeabur 也正在[逐步淘汰共享集群](https://zeabur.com/changelogs/phasing-out-shared-cluster)，新项目请使用专用服务器或专用集群。
</Callout>

本文档概述了在 Zeabur 上部署服务，以实现高可用性的推荐架构。

请注意，这个主题较为高级。对于您的新创服务，通常不需要高可用性（HA）。最简单的解决方案是将您的服务放在一个项目中，并使用[内网](/deploy/private-networking)进行通信；使用[公网](/deploy/public-networking)将您的服务暴露到互联网。我们可以为您处理垂直扩展。
