...
在 [Zeabur Dash](https://zeabur.com/new) 创建 Project。

## 步骤二：创建服务

在上述 Project 中分别创建 NGINX 和 Vaultwarden 的 Prebuilt Service。

![创建 NGINX 服务](/data-management/config-edit/nginx.png)

![创建 Vaultwarden 服务](/data-management/config-edit/vaultwarden.png)

## 步骤三：编辑 Nginx 服务配置文件

1. 在 Services 区中，点击 NGINX 服务的 `Settings` 区块的下拉按钮，找到 `Configs` 部分后点击 `Open Config Editor` 按钮，进入配置文件编辑器。

![打开配置文件编辑器](/data-management/config-edit/nginx_open_config.png)
...
![新增配置文件](/data-management/config-edit/nginx_create_config_file.png)
...
![保存配置文件](/data-management/config-edit/Open_Config_Editor.png)

## 步骤四：重新启动服务

在上述步骤中，我们已经完成了配置文件的编辑，接下来我们需要重新启动服务。点击服务状态的“重新启动服务”按钮重新启动服务，以使配置文件生效。

![重新启动服务](/data-management/config-edit/restart_nginx_Instance.png)
