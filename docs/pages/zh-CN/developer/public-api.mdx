...
若您发现您需要的 API 不在这个 Schema 中，请到我们的 [community forum](https://zeabur.com/forum) 和我们告知。

## 团队拥有的资源

您的 API 密钥绑定在您的**个人账号**上——团队并没有自己的 API 密钥。同一把密钥可以授权访问您所属的任意团队中的资源，访问范围受限于您在该团队中所担任角色的权限。

一个资源（例如项目）可以由您的个人账号拥有，也可以由某个团队拥有。操作这类拥有型资源的查询和变更都接受一个 `ownerID` 参数：传入某个团队的 ID 即可针对该团队进行操作，省略该参数则针对您的个人账号进行操作。例如，要列出某个团队的项目：
...
<Callout type="info">
正在使用 Zeabur CLI？它将这一概念封装为工作区（workspace）——请参阅 CLI 文档中的 [工作区（团队）](./cli#工作区团队)。
</Callout>
