...
目前，您已成功部署 **前端**（视觉界面）。为了启用数据和身份验证，您将连接 PostgreSQL 数据库并将连接详细信息添加为 Zeabur **变量**。

我们将发布专门的后续教程，展示如何在 Zeabur 上创建 PostgreSQL 数据库并将其连接到您的应用程序。如果您现在需要托管选项，也可以使用 Supabase。

**👉 按照 [此指南](https://zeabur.com/templates/B20CX0) 在 Zeabur 上创建并连接 PostgreSQL。**
