...
这里我们使用 [NGINX](https://nginx.com/) 反向代理内网的 [vaultwarden](https://github.com/dani-garcia/vaultwarden) 服务，来演示配置文件编辑器更改 NGINX 配置文件的流程。
...
1. 在 Services 区中，点选 NGINX 服务的 `Settings` 区块的下拉按钮，找到 `Configs` 部分后点选 `Open Config Editor` 按钮，进入配置文件编辑器。

![打开配置文件编辑器](/data-management/config-edit/open-config-editor.en-US.png)

2. 在打开的编辑器中，点选 `Add Config file` 按钮新增一个配置文件。在弹出表单的 `Config Path` 输入框打上配置文件的路径和名称，这里我们输入 `/etc/nginx/nginx.conf`，接着点选 `Save Config` 按钮。
...
```
为了让其他人可以通过 NGINX 服务访问我们的内部 Vaultwarden 服务，我们在 NGINX 中使用 `proxy_pass http://vaultwarden.zeabur.internal:80;` 指令，表示让 NGINX 将连接到此处的所有请求都代理到 Vaultwarden。我们利用 [私有网络](/deploy/networking/private-networking) 功能，在我们的私有网络中通过固定主机名称访问 Vaultwarden。
...
在上述步骤中，我们已经完成了配置文件的编辑，接下来我们需要重新启动服务。点选服务状态的「重新启动服务」按钮重新启动服务，以使配置文件生效。

![重新启动服务](/data-management/config-edit/restart-service.en-US.png)

[将域名绑定到 NGINX 服务后](/deploy/networking/public-networking)，即可通过域名访问 Vaultwarden 服务。

![访问 Vaultwarden](/data-management/config-edit/vultwarden-service.en-US.png)

## 在 Template YAML 中预先指定配置文件

您可以在「服务规格」中使用 `configs` 来预先指定这个服务的配置文件。相关信息可参考[《从 YAML 创建模板》](/template/template-format)一文。
