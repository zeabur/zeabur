...
硬盘的价格信息可参考 [我们的价目表](/pricing/pricing-plans)。
...
- Zeabur 平台的服务支持「零停机重新启动」。当你重新启动服务时，通常不会造成服务中断，几乎感受不到影响。
- 一旦启用 Volumes（持久存储空间）功能，服务就**无法支持零停机重新启动**。每次重新启动时，服务会先完全关闭再重新启动，会造成短暂的服务中断。

## 挂载硬盘

打开您想要挂载硬盘的服务页面，点选「硬盘」分页，您会看到一个「Mount Volumes」的按钮。点选之后，您会看到两个字段：

![mount volumes](/data-management/volumes/mount-volumes.webp)

- **Volume ID** 是这个硬盘的专属代号，你可以用一个简单的名字（比如 `data`、`cache`、`photos`）来方便代表这个硬盘。除此之外，这个代号跟实际挂载的结果无关。
- **Mount Directory** 是硬盘要挂载的目录，简单来说就是「需要持久存储的文件夹」。假如服务中你想要持久存储 `/data` 这个目录，就在这里输入 `/data`。

<Callout type="info">
    Volume 仅属于单个服务，无法跨服务共享。在另一个服务中使用相同的 Volume ID，会创建另一个独立的 Volume，而不是挂载同一块硬盘。如果多个服务需要访问相同的数据，请使用 MinIO 或 RustFS 等可通过网络访问的对象存储服务。
</Callout>
...
如果您想要停止计费某个持久存储空间的费用，请点选硬盘旁边的「删除」按钮。删除后就会停止这个空间的计费。注意删除后，这个目录的数据会 **全部清空** 且 **无法恢复**。

![unmount volumes](/data-management/volumes/unmount-volumes.webp)

## 在 Template YAML 中使用硬盘

您可以在「服务规格」中使用 `volumes` 来挂载硬盘。相关信息可参考[《从 YAML 创建模板》](/template/template-format)一文。
