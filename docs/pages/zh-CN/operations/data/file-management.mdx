...
<Callout type="warning">
  某些容器过于精简，以至于不支持 File Management 必须的组件（cat 等），因此可能无法使用此功能。
</Callout>

## 打开文件管理

在服务页面的 **服务状态** 分页中，点选 **文件** 区块即可打开文件管理界面。

![file management](/data-management/file-management/file-management.png)

文件管理界面由五个区块组成：(1) 创建文件夹、(2) 创建文件、(3) 刷新、(4) 目录列表以及 (5) 文件内容。其中，刷新功能可在文件更新后重新加载文件列表；目录列表显示服务的完整目录架构，用户可通过文件夹左侧的箭头展开或收合子目录；而文件内容区块则提供文件内容的查看与编辑功能。

## 创建文件和文件夹

创建文件夹时，点选「创建文件夹」按钮后，输入完整的文件夹路径，再按下打勾键即可完成。如需取消操作，只要将路径保持空白并点选返回按钮即可。

![create folder](/data-management/file-management/create-folder.png)

创建文件的步骤类似，点选「创建文件」按钮，输入完整的文件路径后按下打勾键即可。同样地，若要取消创建，将路径保持空白并点选返回按钮即可。
...
用户可以直接在文件内容区块中进行编辑，完成后按下右上角的保存按钮即可保存变更。保存成功后会在右下角显示确认消息。
...
将鼠标光标移至文件项目上，点选「删除」按钮即可删除文件。需要注意的是，目前系统在删除前不会显示确认提示，请谨慎操作。

![delete file](/data-management/file-management/delete-file.png)

目前 Zeabur 不支持在界面删除整个文件夹，您可以使用 [命令执行](/deploy/config/command-execution) 功能搭配 `rm` 命令来删除整个文件夹。以下是使用 `rm -rf` 删除整个 `/tmp/testdir` 文件夹的示例：
...
要下载文件，只需将鼠标光标移至文件项目上，点选「下载」按钮即可。系统会在右下角显示下载进度，下载完成后可选择保存位置。
...
上传文件时，只需将文件拖拽到目标目录上即可。系统会以背景色标示目标上传位置，松开鼠标后即开始上传。目前系统仅支持单一文件上传。
...
接着使用 [命令执行](/deploy/config/command-execution) 功能，输入以下指令来查看 `tar` 文件内容：
