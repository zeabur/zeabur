...
<Callout>
    如果您没有挂载 [持久存储空间](/operations/data/volumes)，就不会显示备份功能。「备份」也只会备份「持久存储空间」内的数据，其余服务中的文件（如源代码）不会跟着备份。
</Callout>
...
备份文件包含数据库的标准转储。PostgreSQL 备份使用 `pg_dumpall` 创建，并包含服务中的所有数据库；MySQL 和 MariaDB 备份包含所有非系统数据库；MongoDB 备份包含服务凭据可访问的所有数据库。PostgreSQL、MySQL 和 MariaDB 使用 `.sql` 格式，MongoDB 则包含多个 `.bson` 文件。您可以日后使用 `psql`、`mysql`、`mongorestore` 等数据库还原工具恢复这些文件。
...
如果您要还原 [持久存储空间](/operations/data/volumes) 内的数据，您可以参考 [文件管理](/operations/data/file-management) 的方式，将备份文件上传至目标目录并解压缩。
