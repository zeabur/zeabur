---
title: 注册服务器
ogImageTitle: 注册服务器
ogImageSubtitle: 注册一个服务器到 Zeabur 上
---

import { Callout } from 'nextra/components';

# 注册服务器

本文将介绍如何注册一台 [服务器](../server) 到 Zeabur 上。如果你还没有自己的服务器，可以改为[从 Zeabur 购买](./purchase)。

## 前置条件

- 你已经拥有一台服务器，并且这台服务器必须拥有至少一个公网能够连接的 IP 地址。
- 这台服务器必须至少有 1 个 CPU 核心，且至少有 2 GB 的内存。
- 这台服务器必须对外开放 `22`, `80`, `443`, `4222`, `6443`, `30000-32767` 这些端口。
- 这台服务器必须允许 SSH 登录，且用户名为 `root`。
- 这台服务器必须允许使用 `root` 或其他可以使用 `sudo` 的用户，通过密码或 SSH 密钥登录。

<Callout type="default">
    如果您不是使用 `root` 用户，考虑到 Zeabur 是在非交互环境下配置机器的（无法代为输入密码），请确保您这个用户所在的 `sudo` 用户组已被授予 [`NOPASSWD`](https://linux.die.net/man/5/sudoers) 标记。
</Callout>

## 注册步骤

![add machine to zeabur](/server/add-machine-to-zeabur.webp)

1. 登录 Zeabur 后，点击顶部导航栏或项目列表页面上方的“我的服务器”，或是直接访问 [https://zeabur.com/servers](https://zeabur.com/servers)。
2. 点击页面右上角的“创建”按钮，然后选择“添加自己的服务器”。
3. 输入服务器的 IP 地址及 SSH 端口（默认为 `22`）。
4. 输入用户名（例如 `root`），然后选择登录方式（密码或 SSH 密钥）。
5. 若选择密码，则在下方输入密码，若选择 SSH 密钥，则在下方输入 SSH 密钥。

<Callout type="warning">
    Zeabur 仅接受 **OpenSSH 格式** 的 SSH 密钥（私钥以 `-----BEGIN OPENSSH PRIVATE KEY-----` 开头）。部分云服务商（如腾讯云）默认提供的 SSH 密钥为旧版 RSA PEM 格式（`-----BEGIN RSA PRIVATE KEY-----`），这会导致 SSH 连接失败。您可以使用以下命令将密钥转换为 OpenSSH 格式：

    ```bash
    ssh-keygen -p -N "" -f /path/to/your_private_key
    ```

    转换完成后，将更新后的密钥内容粘贴到 SSH 密钥输入框中。
</Callout>

6. 点击"新增服务器"按钮，等待服务器注册完成（过程约需 30 秒）。
7. 注册成功后，你会在服务器列表看到多了一台刚刚注册的服务器。
8. 稍等大约一分钟直到状态显示为“在线”且 CPU 和内存的使用情况可以正常显示。

若你遇到注册失败的情况，请到 [Zeabur 支持](https://zeabur.com/support) 向我们反馈。

## 维护服务器

请参考我们的 [维护服务器](./operate) 文档，了解如何维护您的服务器。
