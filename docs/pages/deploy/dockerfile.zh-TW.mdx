...
<Callout type="warning">
  Zeabur 目前僅支援 `Dockerfile` 的部署方式，不提供 Docker Compose 的方式。
</Callout>
...
如果你的 `Dockerfile` 只有單個階段（stage），或是你的環境變數只需在最後一個階段中使用，那麼你無需手動寫 `ENV`，Zeabur 會自動幫你加上。

### ARG

但如果你的 `Dockerfile` 使用了[多階段（multi-stage）](https://docs.docker.com/build/building/multi-stage/)，並且需要在建置前就設定環境變數，你可以使用 `ARG` 來設定。

以下我們以 Node.js 及 NGINX 為例，撰寫一個 `Dockerfile` 來部署。
...
## 這裡的 `BUILDTIME_ENV_EXAMPLE` 會自動在建置前設定
...
如果你的 Monorepo 裡有不同專案想要分開部署，你可以新增 `Dockerfile.<服務名稱>`（或 `<服務名稱>.Dockerfile`）檔案，並透過服務名稱部署指定的 Dockerfile。

以這個例子來說，假如你想部署三個微服務，但都使用同一個 Git 儲存庫，你可以這樣命名自己的 Dockerfile：
...
接著點擊「Redeploy」重新部署專案，即可部署你想要的服務。你也可以與 [觸發路徑](./watch-paths) 搭配使用，只在修改相應檔案時才觸發部署。
