...
這裡我們使用 [nginx](https://nginx.com/) 作為前置代理 [Botway CE](https://github.com/botwayorg/ce)，來示範配置檔案編輯器編輯 ngnix 自訂配置檔案的過程。
...
在上述 Project 中分別建立 Prebuilt Service ，服務名稱分別為 `nginx` 和 `botway`。

![建立 Ngnix 服務](/deploy/config-edit/create-nginx-service.en-US.png)

![建立 Botway CE 服務](/deploy/config-edit/create-botwayce-service.en-US.png)
...
3. 根據需求，編輯配置檔案的內容，這裡我們添加了一個 `server` 配置，用於反向代理 Botway CE 服務。
...
```
上面的文件內容中，`proxy_pass` 指令中的 `botway-ce.zeabur.internal` 是 Botway CE 服務的服務名，這裡我們使用了 [Private Network](/deploy/private-networking) 功能，可以透過固定的主機名稱來使用內網訪問服務。
