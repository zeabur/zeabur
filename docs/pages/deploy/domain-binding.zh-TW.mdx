...
**網域**是你的服務的唯一識別碼，它可以用來在網路上存取你的服務。例如，這個網站的網域是 [`zeabur.com`](https://zeabur.com/)。

在你的服務成功部署到 Zeabur 之後，你可能需要綁定一個網域來將這個服務暴露到網路上。

## 步驟一：打開網域設定

選擇你想要為其加入網域的服務，展開 **網域** 區塊，你會看到兩個按鈕，用於為你的服務建立網域。
...
你只需要點擊 **產生網域** 按鈕，然後輸入你想要使用的網域，如果這個網域合法且沒有被其他人使用，你就可以點擊 **儲存** 按鈕，並在之後使用它來存取你的服務。

![generate](/deploy/domain/generate.png)

你也可以選擇完全自訂你的網域，但為此你需要按照 **步驟三** 中的說明進行一些設定。
