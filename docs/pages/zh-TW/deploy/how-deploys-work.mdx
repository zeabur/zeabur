...
title: 建置原理
ogImageTitle: 建置原理
ogImageSubtitle: Zeabur 內部建置使用者部署服務的方式
---

# 建置原理

Zeabur 使用 [zbpack](https://github.com/zeabur/zbpack) 作為內部建置使用者服務的工具，讓使用者可以在不需要理解複雜細節的情況下，一鍵部署基於任何語言和框架的服務。

目前，[zbpack](https://github.com/zeabur/zbpack) 已經支援所有網頁開發主流的程式語言，並對各個較熱門的開發框架實現了進一步的識別及最佳化，同時也在不斷為新出現的程式語言和框架推出更新：
...
若你的服務使用的語言或框架不在上述列表中，或是你發現 zbpack 在建置服務的過程中發生了問題，歡迎到 [GitHub](https://github.com/zeabur/zbpack/pulls) 貢獻 Pull Request，幫助我們完善 zbpack 的功能。

當你在某個專案中使用 zbpack 建置服務時，它會根據專案中的程式碼、設定檔等資訊，自動識別出服務使用的語言和框架，並根據這些資訊，選擇適合的建置方式來建置服務。
...
當你在這個專案的資料夾中執行 `zbpack` 指令，可以看到他為此訂製的建置計畫：
...
從圖中可以看到， `zbpack` 選擇使用 `nodejs` Provider 作為建置的方案提供者，這意味著他在進一步的計畫中會做以下事情：
...
接下來，zbpack 會根據這個建置計畫，產生一個 Dockerfile，然後自動使用這個 Dockerfile 來建置服務：
...
如此一來，我們就可以直接使用 `docker run -p 8080:8080 -it nextjs-template` 來啟動服務，然後在瀏覽器中打開 `http://localhost:8080` 來存取服務。
