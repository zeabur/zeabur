...
如果你在 Apple Silicon Mac（M1/M2/M3/M4）上本地建置 Docker 映像檔，映像檔預設會是 `linux/arm64` 平台。
Zeabur 的伺服器執行在 `linux/amd64` 上，因此部署僅包含 ARM 架構的映像檔會失敗，並顯示 **「no match for platform in manifest」** 錯誤。

為避免此問題，請在建置時加上 `--platform linux/amd64` 參數：
...
除了其他服務 Exposed 的變數以外，你也可以使用 [Zeabur 預先定義好的特殊變數](/deploy/config/environment-variables)，比如使用 `${ZEABUR_WEB_URL}` 取回設定在 `web` port 的網址。
...
「Port Type」可以是 `HTTP` 或 `TCP`。外網的部分，`HTTP` 可以使用網域（如 `my-service.zeabur.app`）直連，同時 Zeabur 也會自動簽署 TLS 憑證；`TCP` 則可以使用 Zeabur 自動分配的轉送主機名稱和 Port 連線（如 `reg.clusters.zeabur.com:12345`）。如果是服務之間，可以使用 [服務內網](/deploy/networking/private-networking) 直連到服務，減少不必要的流量費用，此時 Port 直接填寫你宣告的 Port 即可（亦即 `mariadb.zeabur.internal:3306`、`backend.zeabur.internal:8080`）。
...
從 GUI 建立的 Start Command 預設的 Command 都是 `/bin/sh`。如果你的 Command 不是 `/bin/sh`，可使用 Edit TOML 功能直接修改，或與 [客服](/get-started/faq-support/help) 討論更改方式。

## 撰寫自己的模板 YAML

雖然 GUI 提供了許多用於建立您的 Docker 服務的必要參數，但您可能希望新增自訂圖示、連線說明或詳細設定。在這種情況下，[以程式碼撰寫您的模板](/template/template-format) 可能是更好的選擇。

如果您對於撰寫自己的範本有任何問題，歡迎聯絡[客服](/get-started/faq-support/help)。
