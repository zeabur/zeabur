...
或者是使用 [環境變數](/deploy/config/environment-variables) 進行設定：
...
或者是設定 [環境變數](/deploy/config/environment-variables)：
...
你也可以使用 [環境變數](/deploy/config/environment-variables) 來設定編譯和啟動命令：
