...
硬碟的價格資訊可參考 [我們的價目表](/pricing/pricing-plans)。
...
<Callout type="info">
    Volume 僅屬於單一服務，無法跨服務共享。在另一個服務使用相同的 Volume ID，會建立另一個獨立的 Volume，而不是掛載同一顆硬碟。若多個服務需要存取相同資料，請使用 MinIO 或 RustFS 等可透過網路存取的物件儲存服務。
</Callout>
...
您可以在「服務規格」中使用 `volumes` 來掛載硬碟。相關資訊可參考[〈從 YAML 建立模板〉](/template/template-format)一文。
