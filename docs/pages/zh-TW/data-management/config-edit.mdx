...
在 [Zeabur Dash](https://zeabur.com/) 建立 Project。
...
```
為了讓其他人可以透過 NGINX 服務存取我們的內部 Vaultwarden 服務，我們在 NGINX 中使用 `proxy_pass http://vaultwarden.zeabur.internal:80;` 指令，表示讓 NGINX 將連線到此處的所有請求都代理到 Vaultwarden。我們利用 [私有網路](/deploy/networking/private-networking) 功能，在我們的私有網路中透過固定主機名稱存取 Vaultwarden。
...
[將網域綁定到 NGINX 服務後](/deploy/networking/public-networking)，即可透過網域造訪 Vaultwarden 服務。

![造訪 Vaultwarden](/data-management/config-edit/vultwarden-service.en-US.png)

## 在 Template YAML 中預先指定設定檔

您可以在「服務規格」中使用 `configs` 來預先指定這個服務的設定檔。相關資訊可參考[〈從 YAML 建立模板〉](/template/template-format)一文。
