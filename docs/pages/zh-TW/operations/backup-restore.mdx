...
<Callout>
    如果您沒有掛載 [持久儲存空間](/operations/volumes)，就不會顯示備份功能。「備份」也只會備份「持久儲存空間」內的資料，其餘服務中的檔案（如原始碼）不會跟著備份。
</Callout>
...
如果您要還原 [持久儲存空間](/operations/volumes) 內的資料，您可以參考 [檔案管理](/operations/file-management) 的方式，將備份檔案上傳至目標目錄並解壓縮。
