...
<Callout>
    如果您沒有掛載 [持久儲存空間](/operations/data/volumes)，就不會顯示備份功能。「備份」也只會備份「持久儲存空間」內的資料，其餘服務中的檔案（如原始碼）不會跟著備份。
</Callout>
...
備份檔案包含資料庫的標準傾印。PostgreSQL 備份使用 `pg_dumpall` 建立，並包含服務中的所有資料庫；MySQL 和 MariaDB 備份包含所有非系統資料庫；MongoDB 備份包含服務憑證可存取的所有資料庫。PostgreSQL、MySQL 和 MariaDB 使用 `.sql` 格式，MongoDB 則包含多個 `.bson` 檔案。您可以日後使用 `psql`、`mysql`、`mongorestore` 等資料庫還原工具復原這些檔案。
...
如果您要還原 [持久儲存空間](/operations/data/volumes) 內的資料，您可以參考 [檔案管理](/operations/data/file-management) 的方式，將備份檔案上傳至目標目錄並解壓縮。
