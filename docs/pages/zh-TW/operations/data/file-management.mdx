...
目前 Zeabur 不支援在介面刪除整個資料夾，您可以使用 [命令執行](/deploy/config/command-execution) 功能搭配 `rm` 命令來刪除整個資料夾。以下是使用 `rm -rf` 刪除整個 `/tmp/testdir` 資料夾的範例：
...
接著使用 [命令執行](/deploy/config/command-execution) 功能，輸入以下指令來查看 `tar` 檔案內容：
