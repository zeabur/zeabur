...
<Callout type="info">
  如果你的服務部署在 [Wonder Mesh](/wonder-mesh) 伺服器上，你也可以直接從區域網路存取它，無需網域或通訊埠轉發。詳情請參閱[從區域網路存取服務](/wonder-mesh/get-started#accessing-services)。
</Callout>
...
你可以在 [Docker 服務設定](/deploy/methods/custom-docker-image) 時設定連線埠。詳細資訊請參閱〈Docker 服務設定〉文件中的「Ports」章節。
