...
<Callout type="info">
  此高可用性架構適用於**專用伺服器**和**專用叢集**。但如果想要在單個節點故障時自動轉移到另一個正常的節點（也就是配置多個 Kubernetes 節點），會需要**專用叢集**。Zeabur 也正在[逐步淘汰共享叢集](https://zeabur.com/changelogs/phasing-out-shared-cluster)，新專案請使用專用伺服器或專用叢集。
</Callout>

本文件概述了在 Zeabur 上部署服務，來實作高可用性所推薦的架構。

請注意，這個主題較為進階。對於您的新創服務，通常不需要高可用性 (HA)。最簡單的解決方案是將您的服務放在一個專案中，並使用 [內網](/networking/private) 進行通訊；使用 [公網](/networking/public) 將您的服務暴露到網際網路。我們可以為您處理垂直擴縮。
