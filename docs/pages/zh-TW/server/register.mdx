---
title: 註冊伺服器
ogImageTitle: 註冊伺服器
ogImageSubtitle: 註冊一個伺服器到 Zeabur 上
---

import { Callout } from 'nextra/components';

# 註冊伺服器

本文將介紹如何註冊一台 [伺服器](../server) 到 Zeabur 上。如果你還沒有自己的伺服器，可以改為[從 Zeabur 購買](./purchase)。

## 前置條件

- 你已經擁有一台伺服器，並且這個伺服器必須擁有至少一個公網能夠連線的 IP 位址。
- 這台伺服器必須至少有 1 個 CPU 核心，且至少有 2 GB 的記憶體。
- 這台伺服器必須對外開放 `22`, `80`, `443`, `4222`, `6443`, `30000-32767` 這些埠。
- 這台伺服器必須允許 SSH 登入，且使用者名稱為 `root`。
- 這台伺服器必須允許使用 `root` 或其他可以 `sudo` 的使用者，進行密碼或 SSH 金鑰登入。

<Callout type="default">
    如果您不是用 `root` 使用者，考慮到 Zeabur 是在非互動環境設定機器的（無法代為輸入密碼），請確保您這個使用者所在的 `sudo` 身分組有被授予 [`NOPASSWD`](https://linux.die.net/man/5/sudoers) 標籤。
</Callout>

## 註冊步驟

![add machine to zeabur](/server/add-machine-to-zeabur.webp)

1. 登入 Zeabur 後，點選頂部導航欄或專案列表頁面上方的「我的伺服器」，或是直接訪問 [https://zeabur.com/servers](https://zeabur.com/servers)。
2. 點選頁面右上角的「建立」按鈕，然後選擇「新增自己的伺服器」。
3. 輸入伺服器的 IP 位址及 SSH 連接埠（預設為 `22`）。
4. 輸入使用者（比如 `root`），然後選擇登入方式（密碼或 SSH 金鑰）。
5. 若選擇密碼，則在下方輸入密碼，若選擇 SSH 金鑰，則在下方輸入 SSH 金鑰。

<Callout type="warning">
    Zeabur 僅接受 **OpenSSH 格式** 的 SSH 金鑰（私鑰以 `-----BEGIN OPENSSH PRIVATE KEY-----` 開頭）。部分雲端服務商（如騰訊雲）預設提供的 SSH 金鑰為舊版 RSA PEM 格式（`-----BEGIN RSA PRIVATE KEY-----`），這會導致 SSH 連線失敗。您可以使用以下指令將金鑰轉換為 OpenSSH 格式：

    ```bash
    ssh-keygen -p -N "" -f /path/to/your_private_key
    ```

    轉換完成後，將更新後的金鑰內容貼到 SSH 金鑰輸入框中。
</Callout>

6. 點選「新增伺服器」按鈕，等待伺服器註冊完成（過程約需 30 秒）。
7. 註冊成功後，你會在伺服器列表看到多了一台剛剛註冊的伺服器。
8. 稍等大約一分鐘直到狀態顯示為「線上」且 CPU 和記憶體的使用情況可以正常顯示。

若你遇到註冊失敗的情況，請到 [Zeabur 支援](https://zeabur.com/support) 向我們回報。

## 維護伺服器

請參考我們的 [維護伺服器](./operate) 文件，了解如何維護您的伺服器。