...
<Callout type="warning">
  `html` 和 `text` 至少需要提供一個。如果同時提供，郵件客戶端會優先顯示 HTML 版本，不支援 HTML 的客戶端則顯示純文字版本。總收件人數（to + cc + bcc）不能超過 50 人。
</Callout>
...
<Callout type="info">
各方案的每月寄送配額、超額計費價格與計費方式，請參閱[配額與超額計費](/email/quotas-and-overage)。
</Callout>
