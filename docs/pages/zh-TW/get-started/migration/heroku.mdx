...
4. **更新應用程式的資料庫連線**：在 Zeabur 的 **Variables** 分頁中設定 `DATABASE_URL`。您可以使用 Zeabur 的變數參照 `${POSTGRES_URI}` 來自動注入 Marketplace 服務的連線字串。

<Callout>
若使用 Heroku Redis，可使用 `redis-cli --rdb` 匯出資料，再匯入到 Zeabur Marketplace 中的 Redis 執行個體。
</Callout>

## 步驟 4：替換 Heroku Add-on

Heroku 的 add-on 在 Zeabur Marketplace 中有對應的替代方案，或可作為外部服務連接：
...
| 資料庫 | Heroku Postgres（受管理） | Marketplace（PostgreSQL、MySQL、MongoDB、Redis 等） |
| 附加服務 | Heroku Add-on 市集 | Zeabur Marketplace + 外部服務 |
...
完成了！您的 Heroku 應用程式現在已在 Zeabur 上運行，不再需要 dyno、Procfile 和 slug 大小限制。如果遇到任何問題，歡迎到我們的 [Discord 社群](https://zeabur.com/dc) 尋求協助。
