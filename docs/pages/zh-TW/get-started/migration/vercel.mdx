...
- **後端 API**：在同一個專案中部署獨立的後端服務（Node.js、Python、Go 等）。同專案中的服務可以透過[私有網路](/deploy/networking/private-networking)互相通訊。
...
| 計價模式 | 按 function 呼叫次數 + 頻寬計費 | [訂閱方案](/pricing) + 伺服器費用 |
| 團隊功能 | 按席次計價 | 彈性團隊方案 |
| 基礎架構 | 僅 Serverless | 伺服器；共享叢集僅供既有使用者使用 |

---

完成了！您的 Vercel 應用程式現在已在 Zeabur 上運行，作為一個全端平台，資料庫、後端服務和前端全部整合在同一個地方。如果遇到任何問題，請聯繫 [Zeabur 支援](https://zeabur.com/support)。
