...
<Callout>
Vercel 將變數依環境區分（Production、Preview、Development）。在 Zeabur 上，變數預設會套用到所有部署。如果您需要為預覽部署設定不同的值，可以按分支進行設定。
</Callout>
...
| Edge Functions | 在基礎架構層級處理（內建邊緣快取） |
| Vercel Postgres / KV / Blob | Marketplace → PostgreSQL、Redis 或物件儲存 |
| Cron Jobs（`vercel.json`） | 使用框架內建的 cron 或獨立的 worker 服務 |
| `vercel.json` rewrites/redirects | 使用框架層級的設定（例如 `next.config.js` rewrites） |
| Preview Deployments | 自動為 pull request 建立預覽部署 |
...
- **PostgreSQL、MySQL、MongoDB**：點選 **Add Service** → **Marketplace** 並選擇您的資料庫。
- **Redis**：在 Marketplace 中提供，可用於快取和 session 儲存。
...
| 資料庫 | Vercel Postgres、KV、Blob（有限） | 完整 Marketplace（PostgreSQL、MySQL、MongoDB、Redis 等） |
| 後端服務 | 原生不支援 | 可在前端旁部署任何後端 |
| 建置系統 | Vercel Build | 自動偵測（Nixpacks / Dockerfile） |
| 計價模式 | 按 function 呼叫次數 + 頻寬計費 | 按使用量計算運算資源 + 儲存空間 |
| 團隊功能 | 按席次計價 | 彈性團隊方案 |
| 基礎架構 | 僅 Serverless | 共享叢集 + 專屬伺服器 |

---

完成了！您的 Vercel 應用程式現在已在 Zeabur 上運行，作為一個全端平台，資料庫、後端服務和前端全部整合在同一個地方。如果遇到任何問題，歡迎到我們的 [Discord 社群](https://zeabur.com/dc) 尋求協助。
