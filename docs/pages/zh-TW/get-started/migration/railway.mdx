...
2. **在 Zeabur 上建立**：點選 **Add Service** → **Marketplace** → **PostgreSQL**。
...
2. **在 Zeabur 上建立**：點選 **Add Service** → **Marketplace** → **MySQL**。
...
2. **在 Zeabur 上建立**：點選 **Add Service** → **Marketplace** → **Redis**。
...
| 資料庫 | 內建 Postgres、MySQL、Redis、MongoDB | Marketplace 提供相同選項及更多 |
| 磁碟區 | 每個服務獨立磁碟區 | 每個服務獨立磁碟區 |
| 擴展 | 手動設定副本數量 | 自動擴展 + 手動選項 |
| 專屬伺服器 | 不提供 | 提供（裸機方案） |
...
完成了！您的 Railway 應用程式現在已在 Zeabur 上運行。兩個平台的開發者體驗非常相似，您應該很快就能上手。如果遇到任何問題，歡迎到我們的 [Discord 社群](https://zeabur.com/dc) 尋求協助。
