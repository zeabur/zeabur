...
所有設定值 — API 金鑰、資料庫連線字串、功能開關 — 都應存放在[環境變數](/deploy/config/environment-variables)中。絕對不要把機密資訊寫死在原始碼或 Docker 映像檔裡。
...
[持久儲存空間](/operations/data/volumes)對資料庫和檔案儲存來說不可或缺，但有個取捨：掛載了 Volume 的服務無法執行零停機滾動部署。
