...
Build Templates, Earn Bounties, and Grow the Community!

While Zeabur hosts over a thousand one-click templates, many excellent open-source projects are still missing. To encourage community contributions and expand this library, we are launching the **Zeabur Template Bounty Hunter** program.

### How It Works

#### 1. Users post template requests in the Discord Channel

Go to the `#template` channel on the [Zeabur Discord](https://zeabur.com/dc) and paste the GitHub project link you want to see as a template.

If you can find the project's `docker-compose.yaml`, please include it as well, as this will help the Zeabur team evaluate it faster!

#### 2. Avoid duplicate requests

Before posting, please search the channel to see if the same request already exists.

If someone has already made the request, simply press :arrow_up: UP to show your support. This lets the team know how important the template is to the community.

#### 3. Zeabur team evaluation and pricing

We will evaluate and set a corresponding bounty price based on the template's complexity and the number of upvotes, and we will indicate it in the post's title.

#### 4. Bounty hunters, it's time to act!

If you are a developer who knows how to create Zeabur templates, you are free to "accept" the task and develop the template.

Please write "Claiming this task" under the template request and keep updating your progress so the community knows the development status and to avoid duplicate work.

Once completed and verified by the Zeabur team, you will receive the bounty!

### Tips for Builders

Want to finish your template faster? Here are some suggestions:

- Read the [Zeabur Template Documentation](../template/template-in-code)
- Ensure environment variables (envs), ports, and build commands are clearly configured.
- Test that the template can be deployed successfully with one click.
- You can test with the [Zeabur CLI](https://github.com/zeabur/zeabur/tree/main/cli) locally before submitting.
