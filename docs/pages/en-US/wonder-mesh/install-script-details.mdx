...
- Installs `openssh-server` if it is not already running
...
| Outbound connections | Connects to `tailscale.com`, `cdn.zeabur.com`, `wonder-net.strrl.dev`, `api.zeabur.com`, `ifconfig.me` during installation |

---

## macOS

The script runs as a **regular user** (do not use `sudo`). All mesh networking and system changes are isolated inside a virtual machine — your macOS host configuration is not modified directly.
...
Your macOS SSH configuration, system users, and network settings are **not** modified.

---

## Uninstallation

If you need to completely remove Wonder Mesh from your device:

### Linux

```bash
# Remove the zeabur user and its sudo config
sudo userdel -r zeabur
sudo rm -f /etc/sudoers.d/zeabur

# Restore SSH configuration
sudo rm -f /etc/ssh/sshd_config.d/00-zeabur-password-auth.conf
# Manually review /etc/ssh/sshd_config for PasswordAuthentication setting
sudo systemctl restart sshd

# Disconnect and remove Tailscale
sudo tailscale logout
sudo systemctl disable --now tailscaled
# Remove the tailscale package using your package manager

# Remove the Wonder binary
sudo rm -f /usr/local/bin/wonder

# Clean up logs
rm -f /tmp/zeabur-install.log
```
...
limactl stop zeabur-mesh
limactl delete zeabur-mesh

# Optionally uninstall Lima
brew uninstall lima
```
