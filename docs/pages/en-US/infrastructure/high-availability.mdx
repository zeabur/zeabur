...
Note that this is an advanced topic. Typically, you don't need high availability (HA) for your startup services. For the simplest solution, place your services in one project and use [private networking](/infrastructure/networking/private-networking) for communication. Expose your service to the Internet using [public networking](/infrastructure/networking/public-networking). We can handle vertical scaling for you. In shared clusters, if a node goes down, we can automatically migrate your service to another node.
