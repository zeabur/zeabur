...
If your start command is complex (such as involving the setup of scheduled tasks, starting multiple services, etc.), `start_command` may become too complicated and difficult to debug. You can try packaging these commands into a script using the [Config Editor](/deploy/config-edit), such as `worker.sh`:
...
Using Zeabur’s [configuration file editor](/deploy/config-edit), create a configuration file in the `/usr/local/etc/php/conf.d` directory with the `.ini` extension and include the PHP settings you want to override.
...
Using Zeabur’s [configuration file editor](/deploy/config-edit), create a configuration file at `/etc/nginx/sites-enabled/default` to override the default NGINX settings in Zeabur. The editor will prefill the current NGINX configuration, which you can modify as needed.
