...
- **Gradle**: `build/libs/*.jar` (or `build/libs/*.war`)
...
If your entire Java project (including `pom.xml` or `build.gradle`) lives in a subdirectory of the Git repository, use the [Root Directory](/deploy/config/root-directory) setting instead. This tells Zeabur to treat that subdirectory as the project root, and default JAR discovery will work as normal.
