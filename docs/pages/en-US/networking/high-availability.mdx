...
<Callout type="info">
  This high availability architecture applies to both **Dedicated Servers** and **Dedicated Clusters**. However, horizontal scaling (service replicas) requires a **Dedicated Cluster** (multi-node Kubernetes cluster). Zeabur is also [phasing out shared clusters](https://zeabur.com/changelogs/phasing-out-shared-cluster) — new projects should use Dedicated Servers or Dedicated Clusters.
</Callout>
