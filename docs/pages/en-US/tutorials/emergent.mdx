...
> 📝 Important Note: This guide covers the full stack. We will first deploy your application code, and in Step 7, we will specifically cover how to provision and connect a MongoDB database for apps that need to save user data.
> 
...
Projects usually detect the package manager automatically. Check for a lock file (`pnpm-lock.yaml`, `package-lock.json`, or `yarn.lock`).
...
## Step 7: Connect a Database (MongoDB)

If your Emergent app needs to persist user data, provision a MongoDB service and connect it to your app.

1. Deploy MongoDB on Zeabur using the MongoDB template:
   **👉 [MongoDB template](https://zeabur.com/templates/KXL04P)**
2. In the MongoDB service page, open the **Instruction** / **Connections** tab and copy the connection string (URI).
3. Set your app environment variable (commonly `MONGO_URI`) to that URI:
   - **Local development**: use the **Public** connection string.
   - **Apps deployed on Zeabur**: use the **Internal/Private** connection string (faster and avoids public bandwidth).
