...
ogImageTitle: Z-Send Domain Management
ogImageSubtitle: Verify and Manage Sender Domains
---

# Domain Management

To send emails with Z-Send, you need to verify your sender domain first. Domain verification ensures that you own the domain and improves email deliverability.
...
After adding the DNS records, click the "Verify" button in the console. Z-Send will automatically check your DNS configuration.
...
To ensure emails are delivered successfully, Z-Send uses the following standard email authentication technologies:
...
3. **View error messages**: Z-Send console will show specific verification errors, please modify according to the prompts
...
When the Z-Send console shows the domain status as **`verified`**, it means all DNS records have been correctly configured and passed verification. You can immediately start using this domain to send emails.
