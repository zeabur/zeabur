...
ogImageTitle: Z-Send REST API Reference
ogImageSubtitle: Complete API Endpoint Documentation
---

# REST API Reference

Z-Send provides a complete REST API that makes it easy to integrate email sending functionality into your application.
...
<Callout type="info">
  API keys can be created and managed in the Z-Send management page of the Zeabur console.
</Callout>
...
curl -X POST https://api.zeabur.com/api/v1/zsend/emails \
  ...
  -d '{
    ...
    "html": "<h1>Welcome!</h1><p>Thank you for using Z-Send.</p>"
  }'
```
...
Same as single email sending, with an additional field:

```json
{
  // ... other fields same as /emails
  "scheduled_at": "2026-01-25T10:00:00Z"  // ISO 8601 format
}
```
...
<Callout type="info">
  A single batch send supports up to 100 emails. Each email has the same fields as single email sending.
</Callout>
