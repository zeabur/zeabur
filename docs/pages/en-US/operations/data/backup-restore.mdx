...
ogImageTitle: Backup & Restore
ogImageSubtitle: Backup and Restore your Service on Zeabur
---
...
<Callout>
    If you have not mounted [volumes](/operations/data/volumes), the backup feature will not be displayed. "Backup" will only back up the data within the volumes; other files in the service (such as source code) will not be backed up.
</Callout>
...
The backup file contains a standard database dump. PostgreSQL backups are created with `pg_dumpall` and include all databases in the service. MySQL and MariaDB backups include all non-system databases. MongoDB backups include all databases accessible with the service credentials. PostgreSQL, MySQL, and MariaDB backups use the `.sql` format, while MongoDB backups contain multiple `.bson` files. You can restore these files later with database restoration tools such as `psql`, `mysql`, and `mongorestore`.
...
Currently, Zeabur does not yet support backing up to your own Amazon S3 storage space, but you can write a program to retrieve backups by calling the [Zeabur Public API](/operations/developer-tools/public-api) and store them in your own storage space of your choice:
...
If you want to restore data within [volumes](/operations/data/volumes), you can refer to the [File Management](/operations/data/file-management) method to upload the backup files to the target directory and decompress them.
