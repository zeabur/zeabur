...
Currently, File Management does not support deleting entire folders through the interface. You can run the `rm` command to delete entire folders using [Command Execution](/deploy/config/command-execution). Here is an example of using `rm -rf` to delete the entire `/tmp/testdir` folder:
...
Then run the following command with [Command Execution](/deploy/config/command-execution) to display the file hierarchy of the `tar` file:
