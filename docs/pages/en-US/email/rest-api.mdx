...
| `html` | string | No* | HTML email body. Supports rich formatting such as bold text, colors, images, and links. Max 5 MB. |
| `text` | string | No* | Plain text email body with no formatting. Max 5 MB. |
...
<Callout type="warning">
  At least one of `html` or `text` must be provided. If both are provided, HTML takes priority. Total recipients (to + cc + bcc) cannot exceed 50.
</Callout>
...
| `job_type` | string | Filter by source: `direct`, `scheduled`, `batch` |
| `job_id` | string | Filter by job ID (scheduled email ID or batch job ID) |
...
| Field | Type | Description |
|-------|------|-------------|
| `job_type` | string | Email source: `direct` (single send), `scheduled` (scheduled send), `batch` (batch send) |
| `job_id` | string | Job ID. Empty for direct emails; scheduled email ID or batch job ID otherwise |
...
Configure Webhooks to receive real-time email status updates instead of polling the query interface. See [Webhook Configuration](/en-US/email/webhooks) for details.
