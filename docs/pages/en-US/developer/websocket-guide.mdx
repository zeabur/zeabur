...
2. **Using API Key** - For programmatic access, you can use your [API key](./api-keys) as the authentication token.
...
4. **Use API Keys for server-side** - For backend applications, use [API keys](./api-keys) instead of session tokens
