...
If your Dockerfile is not in the build root, or its filename doesn't follow the `*.Dockerfile` / `Dockerfile.*` convention, use this variable to specify the path. The path is **relative to the service's build root** — that is, the Root Directory configured on the service, or the repository root if none is set:
