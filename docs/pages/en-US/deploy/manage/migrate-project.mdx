...
Migrating a project is essentially **copying the project + syncing data**. Zeabur's [Copy Project](/deploy/manage/copy-project) feature lets you duplicate an entire project (including service configurations, environment variables, and volume data) to another region with one click. Combined with [Backup & Restore](/operations/data/backup-restore), you can perform a complete migration across regions or even across accounts.
...
For detailed steps, see the [Copy Project](/deploy/manage/copy-project) documentation.
...
1. **Export the project configuration** — Use the [Export Project](/deploy/manage/export-project) feature to download the project's YAML configuration.
2. **Back up data** — Create backups for all stateful services:
   - Databases: Use the online backup feature in [Backup & Restore](/operations/data/backup-restore).
   - Persistent storage: Use offline backup or manually download via [File Management](/operations/data/file-management).
...
- **Git integration** — If services are linked to a GitHub repository, you'll need to reconfigure [GitHub integration](/deploy/methods/github-integration) in the new account after cross-account migration.
