...
<Callout>
    Note that "Export Project" **does not include data**. For information on data backup and restoration, please refer to the documentation, [Backup Service](/operations/data/backup-restore) and [Restore Backup](/operations/data/backup-restore)
</Callout>
...
Then, you will see the [Template YAML](/template/template-format) structure of your project. You can choose to directly "Copy" or click "Export" to output it as a YAML file.
...
You will need to make adjustments to the template content (such as `description`, `icon`, `readme`, etc.). Please refer to the instructions on [Creating a Template from YAML](/template/template-format) to update your template and then use the CLI to publish it.
