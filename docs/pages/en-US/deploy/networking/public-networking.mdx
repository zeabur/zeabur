...
<Callout type="info">
  If your service is deployed on a [Wonder Mesh](/wonder-mesh) server, you can also access it directly from your local network without a domain or port forwarding. See [Accessing Services from Your LAN](/wonder-mesh/get-started#accessing-services) for details.
</Callout>
...
You can set up the ports during [the configuration of Docker service](/deploy/methods/custom-docker-image). Check the “Ports” section in the Docker configuration docs for details.
