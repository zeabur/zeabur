...
If your application generates or requires large files at runtime — such as user uploads, cache files, or databases — use [Persistent Volumes](/data-management/volumes) instead of baking them into the container image.
...
- **Split large repositories** — if your repository contains many independent services, consider splitting them into separate repositories and using [Watch Paths](/deploy/config/watch-paths) or [Root Directory](/deploy/config/root-directory) to manage deployments.
