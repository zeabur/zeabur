...
Yes. You need an active subscription plan (at least the Developer Plan) in addition to purchasing or registering a dedicated server. The plan provides access to Zeabur's platform features, while the server provides the computing resources. See [Plans](/billing/plans) for details.

### I only want to host small websites — do I need both a plan and a server?

Yes. To deploy services on Zeabur, you need a subscription plan. If you want to use a dedicated server, you will need to purchase or register the server separately on top of your plan. The Developer Plan's US$5 monthly free quota applies only to shared cluster usage — it does not cover dedicated server costs.
