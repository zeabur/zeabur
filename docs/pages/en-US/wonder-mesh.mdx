...
<Callout type="info">
  Ready to try Wonder Mesh? See the [Get Started guide](/wonder-mesh/get-started) to set up your first Wonder Mesh server.
</Callout>
...
- **From the internet via Gateway**: Set up a [Gateway](/wonder-mesh/public-access) to expose your Wonder Mesh services through a dedicated server with a public IP. This lets you bind domains and receive HTTPS traffic from anywhere.
- **From the internet via port forwarding**: If your network supports it, bind a domain to your service just like any other Zeabur server. See [Public Networking](/deploy/networking/public-networking) for details.
...
Services in the same project can also communicate with each other using internal hostnames. See [Private Networking](/deploy/networking/private-networking) for details.
