...
Create a Project in [Zeabur Dashboard](https://zeabur.com/).
...
To allow others to access our internal Vaultwarden service through the NGINX service, we use the `proxy_pass http://vaultwarden.zeabur.internal:80;` directive in NGINX, indicating that NGINX should proxy all requests to this location to Vaultwarden. We use the [private network](/deploy/networking/private-networking) feature to access Vaultwarden within our private network using a fixed hostname.
...
[After binding the domain to the NGINX service](/deploy/networking/public-networking), you can access the Vaultwarden service through the domain.
...
You can use `configs` in the "Service Specification" to pre-specify the configuration files for this service. For more information, refer to the article, [Create Template from YAML](/template/template-format).
