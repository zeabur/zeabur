...
For pricing information on Volumes, please refer to [our pricing page](/pricing/pricing-plans).
...
<Callout type="info">
    Volumes are scoped to a single service and cannot be shared between services. Reusing the same Volume ID in another service creates a separate Volume; it does not mount the same disk. If multiple services need access to the same data, use network-accessible object storage such as MinIO or RustFS.
</Callout>
...
You can use `volumes` in the "Service Specification" to mount Volumes. For more information, please refer to the article, [Creating Templates from YAML](/template/template-format).
