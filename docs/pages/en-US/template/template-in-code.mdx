...
## Deleting Templates Using the `zeabur` CLI

If you have duplicate templates or no longer need a template, you can delete it using the `npx zeabur@latest template delete` command:
...
Simply run the command and enter the template code when prompted. The template will be permanently removed from the platform.
