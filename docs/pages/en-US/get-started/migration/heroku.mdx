...
4. **Update your app's database URL**: set `DATABASE_URL` in the Zeabur **Variables** tab. You can use Zeabur's variable reference `${POSTGRES_URI}` to automatically inject the connection string from the marketplace service.

<Callout>
For Heroku Redis, use `redis-cli --rdb` to export and import into a Zeabur Redis instance from the marketplace.
</Callout>

## Step 4: Replace Heroku Add-ons

Heroku add-ons have equivalents in Zeabur's marketplace or can be connected as external services:

| Heroku Add-on | Zeabur Equivalent |
|---|---|
| Heroku Postgres | Marketplace → PostgreSQL |
| Heroku Redis | Marketplace → Redis |
...
| Sleep behavior | Free/Basic dynos sleep after 30 min | No sleep — always running |
| Slug size limit | 500 MB compressed | No slug size limit |
| Databases | Heroku Postgres (managed) | Marketplace (PostgreSQL, MySQL, MongoDB, Redis, etc.) |
| Add-ons | Heroku Add-on marketplace | Zeabur Marketplace + external services |
