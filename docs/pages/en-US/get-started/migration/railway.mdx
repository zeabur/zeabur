...
2. **Create on Zeabur**: click **Add Service** → **Marketplace** → **PostgreSQL**.
...
2. **Create on Zeabur**: click **Add Service** → **Marketplace** → **MySQL**.
...
2. **Create on Zeabur**: click **Add Service** → **Marketplace** → **Redis**.
...
| Databases | Built-in Postgres, MySQL, Redis, MongoDB | Marketplace with the same options + more |
| Volumes | Per-service volumes | Per-service volumes |
| Scaling | Manual replica count | Auto-scaling + manual options |
| Dedicated servers | Not available | Available (bare-metal options) |
| Templates | Railway templates | [Zeabur Templates Marketplace](https://zeabur.com/templates) |
