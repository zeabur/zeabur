...
[Persistent volumes](/operations/data/volumes) are essential for databases and file storage, but they come with a trade-off: services with volumes attached cannot perform zero-downtime rolling deploys.
...
- Use the [backup and restore](/operations/data/backup-restore) feature for database snapshots.
- Export important configurations via [config file management](/operations/data/config-file-management).
- Store backups externally (e.g., object storage) for disaster recovery.

<Callout type="info">
Combining these practices with Zeabur's built-in features — like [high availability](/deploy/networking/high-availability) — gives you a solid foundation for production workloads.
</Callout>
